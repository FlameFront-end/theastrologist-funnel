...
const DARK_STEPS = new Set([3, 5, 8, 12, 19, 20, 24, 25, 28])
...
function renderStep(step: number, answers: Record<string, unknown>, go: (step: number, patch?: Record<string, unknown>) => void, back: () => void, restart: () => void) {
  if (step === 1) {
  ...
  switch (step) {
    ...
    case 6: return <SliderScreen question="Какими обычно бывают твои отношения с другими людьми?" states={['Истощающими', 'Сложными', 'Вызывающими смешанные чувства', 'Лёгкими', 'Поддерживающими'].map((label, index) => ({ label, value: ['draining', 'difficult', 'mixed', 'easy', 'supportive'][index], image: `/images/quiz/q8-fig-${index + 1}.webp` }))} initialValue={answers.relationship as string | undefined} onContinue={value => go(7, { relationship: value })} onBack={back} progress={7 / 28} />
    ...
    case 13: return <DateWheelScreen question="Когда у тебя день рождения?" subtitle="По дате рождения мы составим твою натальную карту — основу всего, что покажет тебе Astrologist." initialValue={answers.birthDate} onContinue={birthDate => go(14, { birthDate })} onBack={back} progress={14 / 28} />
    ...
    case 16: return <BirthPlaceScreen initialValue={answers.birthPlace} onContinue={birthPlace => go(17, { birthPlace })} onBack={back} progress={17 / 28} />
    case 17: return <BirthRevealScreen birthDate={answers.birthDate} birthTime={answers.birthTime as string | undefined} onContinue={() => go(18)} onBack={back} progress={18 / 28} />
    case 18: return <PalmGuideScreen onContinue={photo => go(19, { photo })} onBack={back} progress={19 / 28} />
    case 19: return <PalmAnalysingScreen onContinue={() => go(20)} onBack={back} progress={20 / 28} />
    case 20: return <PalmRevealScreen onContinue={() => go(21)} onBack={back} progress={21 / 28} />
    case 21: return <NameScreen answers={answers} onContinue={fullName => go(22, { fullName })} onBack={back} progress={22 / 28} />
    case 22: return <ExpertScreen onContinue={() => go(23)} onBack={back} />
    case 23: return <SocialWallScreen onContinue={() => go(24)} onBack={back} />
    case 24: return <EmailScreen onContinue={email => go(25, { email })} onBack={back} />
    case 25: return <AnalysisScreen onContinue={() => go(26)} onBack={back} />
...
export function QuizFlow() {
  const { step, history, answers, direction, go, back, restart } = useQuiz()
  const content = step === 0 ? <StartScreen onPick={gender => go(1, { gender })} /> : renderStep(step, answers, go, back, restart)
